...
const counts={overnight:260,swing:240};
...
export async function buildSyntheticDb(m,db,days,{random=()=>0}={}) {
 let day='20260104';
 for(let d=0;d<days;d++) {
  ...
  for(const strategy of ['overnight','swing']) {
   const candidates=Array.from({length:counts[strategy]},(_,i)=>({code:String(1000+i),name:'合成候選',exchange:i%2?'TPEx':'TWSE',candidateRank:i+1,price:100,source:(i%2?'TPEx':'TWSE')+' OpenAPI',sourceAsOf:day,volume:10000+Math.floor(random()*1000),changePct:1}));
   ...
   const calendar={tradingDays:[day,...sessions],coveredMonths:months,through:sessions.at(-1),source:m.fixedBenchmarkSpec(strategy).calendarVersion,computedAt:'2026-03-01T08:00:02Z',monthEvidence:Object.fromEntries(months.map(month=>[month,{source:'TWSE FMTQIK',requestedAt:'2026-03-01T08:00:00Z',observedAt:'2026-03-01T08:00:01Z',coveredFrom:month+'01',coveredThrough:m.addDaysCompact(m.addMonthsCompact(month+'01',1),-1),completeMonth:true,status:'fresh'}]))};
   const spec=m.fixedBenchmarkSpec(strategy), modelKey=m.benchmarkModelKey(capture,spec);
   const observations=candidates.map(candidate=>m.buildFixedHorizonObservation({capture,candidate,calendar,rows:[day,...sessions].map(date=>({date,code:candidate.code,exchange:candidate.exchange,open:100,high:102,low:99,close:date===day?100:101,exchangePreviousClose:100,source:candidate.exchange==='TWSE'?'TWSE STOCK_DAY':'TPEx tradingStock',observedAt:'2026-03-01T08:00:00Z'}))}));
...
try {
 ...
 await buildSyntheticDb(m,db,days);
 ...
 const summary=()=>['overnight','swing'].map(s=>m.summarizeVerificationBenchmarks(db,s,{asOf:'20260301'}));
 ...
 console.log(JSON.stringify({fixture:{days,candidatesPerDay:counts,signalsPerDay:{overnight:20,swing:40},retainedRows:{overnight:2,swing:15},fullPublicationsAndManifests:true,completedBenchmarkMemos:days*2},
  ...
  upstreamCalls:srv.mock.calls.length,limits:'synthetic-only; full canonical pools but simplified picks; single-run wall times; legacy loadWithImportMs excludes startServer/O04 identity (use --o08 for readiness); summary repeat is computation, not a result-cache hit; no automatic migration of existing raw memos'}));
...
if(process.argv[1] && resolve(process.argv[1])===resolve(fileURLToPath(import.meta.url))) {
 if(process.argv[2]?.startsWith('--o08')) { const {runMeasurementCli}=await import('./verification-measurement.mjs');await runMeasurementCli(process.argv.slice(2),{buildSyntheticDb}); }
 else await legacyDiagnostic();
}
