...
const APP_SHELL_VERSION = "stock1-shell-v50";
...
function renderPersonalBackupPanel() {
  ...
  if (!authState.user) {
    return `
      ...
      <p>登入後可下載只屬於目前帳號的自選股、到價提醒、交易帳本、個人交易計畫與自己留下的共享備註。</p>
  ...
  return `
    ...
    <p>目前帳號：<strong>${escapeHtml(currentName)}</strong>。下載檔只包含這個帳號的自選股、到價提醒、交易帳本、個人交易計畫等個人資料；券商憑證、密碼、登入工作階段與其他使用者資料不會匯出。</p>
    <div class="personal-backup-panel-grid">
      <article>
        <i data-lucide="download"></i>
        <div>
          <strong>下載個人備份</strong>
          <span>目前格式為 v2，包含個人交易計畫與修改歷史；存成 UTF-8 JSON 後可離線保管。</span>
        </div>
      ...
      <article>
        ...
        <div>
          <strong>從備份復原</strong>
          <span>先做伺服器安全預覽，確認版本、取代範圍與目前密碼後才會寫入。</span>
        </div>
    ...
    <p class="personal-backup-panel-note"><strong>復原規則：</strong>v2 的自選股、到價提醒、交易帳本與交易計畫會取代目前內容；舊 v1 缺少交易計畫時會保留目前計畫。你自己的共享備註只合併，不會刪除或擠掉別人的備註；公司簡介不會匯入。所有版本都先預覽，確認後才寫入。</p>
  `;
}
        ...
        ${topRows
          .map((row) => `
            <button class="verify-chip ${Number.isFinite(row.currentReturn) ? (row.currentReturn >= 0 ? "is-up" : "is-down") : ""}" data-overnight-code="${escapeHtml(row.code)}" data-overnight-exchange="${escapeHtml(row.exchange || '')}" type="button">
              ...
              ${row.holdingOutcomes ? `<small class="holding-outcome ${toneFromNet(row.holdingOutcomes.close?.netPnl)}">${holdingOutcomeText(row.holdingOutcomes.close)}</small>` : ''}
            </button>
          `)
          .join("")}
      </div>
      ${summary.holdingCoverage ? `<p>假設 1 股、不再投入：開盤含息淨報酬 ${formatSignedPercent(summary.holdingMetrics?.avgOpenReturnNet?.value)}；收盤含息淨報酬 ${formatSignedPercent(summary.holdingMetrics?.avgCloseReturnNet?.value)}。開盤 ${holdingCoverageText(summary.holdingCoverage.open)}；收盤 ${holdingCoverageText(summary.holdingCoverage.close)}。原價格觀察與觸價事件另列；模型成本為原始價款的 0.471%，未計個人股利稅／補充保費，公告應收不代表實際入帳。</p>` : ''}
...
function holdingOutcomeText(outcome) {
  if (!Number.isFinite(outcome?.netPnl)) return '含息持有：未知（事件、來源或成本未完整）';
  const pnl = `${outcome.netPnl > 0 ? '+' : ''}${formatNumber(outcome.netPnl, 3)}`;
  return `含息持有 ${Number.isFinite(outcome.holdingReturnPct) ? formatSignedPercent(outcome.holdingReturnPct) : '報酬率未定義'}；模型損益 ${pnl} 元`;
}

function holdingCoverageText(coverage) {
  if (!coverage) return '';
    ...
    ${(data.models || []).map(model => {
      ...
      return `<div class="verification-model"><p>${model.horizon === 'next-open-to-session-15-close' ? '訊號後第 1 個官方交易日開盤 → 第 15 個官方交易日收盤' : '訊號日凍結官方收盤 → 次一官方交易日收盤'}。</p>
        ...
        <details ${verificationFoldAttributes(key)}><summary data-verification-summary="${escapeHtml(key)}">候選池覆蓋、進度與發布時間</summary>
          <p>原採集模型：${verificationIdentityNote(model.captureIdentity || {})}</p>
...
function renderVerificationMeasurement(data, strategy) {
  ...
  const holding = head.identity?.returnBasis === 'cash-holding-return';
  ...
  return `<div class="verification-measurement">
    ...
    <details class="verification-denominators" ${verificationFoldAttributes(`${strategy}:denominators`)}><summary data-verification-summary="${strategy}:denominators">分母、模型與來源</summary>
      ...
      ${(cohort.models || []).map(model => `<div class="verification-model"><strong>${escapeHtml(model.identity?.entryModel || '模型未知')}</strong>
        ...
        ${model.holdingCoverage ? `<p>${strategy === 'swing' ? holdingCoverageText(model.holdingCoverage) : `開盤 ${holdingCoverageText(model.holdingCoverage.open)}；收盤 ${holdingCoverageText(model.holdingCoverage.close)}`}</p>` : ''}
...
function renderVerifyHistory() {
  ...
  const rows = data.records
    .map((record) => (record.status !== "final" || record.complete === false)
      ...
      `)
    .join("");
  const totals = data.cohort?.headline || data.totals;
  const holding = totals?.identity?.returnBasis === 'cash-holding-return';
  ...
  const sufficient = field => (totals?.metricCoverage?.[field]?.validDays ?? totals?.days ?? 0) >= minDays;
  const ciText = (ci, field) => (sufficient(field) && ci && Number.isFinite(ci.low) && Number.isFinite(ci.high)
...
function renderSwingVerifyPanel() {
  ...
  const measured = Boolean(data.cohort);
  const scenarios = data.cohort?.headline?.scenarios || data.scenarios || [];
  ...
  const chips = scenarios
    .map((s) => {
      if (measured) {
        const metric = s.metricCoverage?.netProfitRate;
        const rate = s.netProfitRate == null ? `累積中 ${metric?.validCount || 0}/${s.winRateMinSamples || 20}` : `${formatNumber(s.netProfitRate, 1)}%`;
        return `<div class="sv-chip"><div class="sv-chip-top"><strong>${escapeHtml(swingScenarioName(s.scenario))}</strong>
          <span class="sv-rate"><small>淨獲利率</small> ${rate}</span></div>
          <small>成熟 ${s.matureCount || 0}・未成熟 ${s.immatureCount || 0}・未知 ${s.unknownCount || 0}；達標率 ${s.targetHitRate == null ? '--' : `${formatNumber(s.targetHitRate,1)}%`}；已估值 ${s.continuousResolved || 0}（達標 ${s.wins || 0}・停損 ${s.losses || 0}・超時 ${s.expired || 0}）・待補 ${s.pending || 0}</small>
          <small>${data.cohort?.headline?.identity?.returnBasis === 'cash-holding-return' ? '含息持有平均淨報酬' : '歷史平均淨報酬'} ${formatSignedPercent(s.avgResultPctNet)}・${verificationMetricNote(s.metricCoverage?.avgResultPctNet)}</small>
          ${s.holdingCoverage ? `<small>${holdingCoverageText(s.holdingCoverage)}</small>` : ''}
          ${swingDistributionLine({ ...s, withPeriodicCall: null })}
          ${Object.entries(s.byRegime || {}).map(([key,value]) => `<small>${key === 'aboveMa60' ? '季線上' : key === 'belowMa60' ? '季線下' : '位階未知'}：有效 ${value.metricCoverage?.netProfitRate?.validCount || 0} 筆／${value.metricCoverage?.netProfitRate?.validDays || 0} 日，淨獲利率 ${value.netProfitRate == null ? '--' : `${formatNumber(value.netProfitRate,1)}%`}</small>`).join('')}
          ${s.periodicCallSamples ? `<small>含處置股觀察：${verificationMetricNote(s.withPeriodicCall?.metricCoverage?.avgResultPctNet)}</small>` : ''}
        </div>`;
      }
  ...
  const recentRows = recentAll
    ...
    .map((r) => {
      ...
      const tone = !Number.isFinite(r.resultPct) ? '' : r.resultPct >= 0 ? "is-up" : "is-down";
      return `
        <div class="sv-row">
          ...
          <span class="sv-outcome ${tone}">${label} ${r.holdingOutcome ? '價格 ' : ''}${r.resultPct != null ? (r.holdingOutcome ? formatSignedPercent(r.resultPct) : `${r.resultPct >= 0 ? '+' : ''}${r.resultPct}%`) : ''}${r.holdingOutcome ? `<small class="holding-outcome ${toneFromNet(r.holdingOutcome.netPnl)}">${holdingOutcomeText(r.holdingOutcome)}</small>` : ''}</span>
        </div>`;
    })
    .join("");
  ...
  const foldSummary = (() => {
    if (measured) return `成熟訊號${data.cohort.headline?.identity?.returnBasis === 'cash-holding-return' ? '假設含息持有' : '價格觀察'}・${data.cohort.headline?.matureCount || 0} 筆・展開看場景`;
  ...
  panel.innerHTML = `
    <div class="sv-head">
      <strong>${measured ? `場景成績（成熟訊號${data.cohort.headline?.identity?.returnBasis === 'cash-holding-return' ? '假設含息持有' : '價格觀察'}）` : '場景勝率（前向驗證）'}</strong>
      <small title="每天的選股依官方日 K 逐日對答案：先碰目標＝達標、先碰結構停損＝停損；同一天兩邊都碰到，保守記停損。漏開 App 會按日期補判，中間缺 K 則停住、不跳日。&#10;處置期間的標的是分盤集合競價（每 5 或 20 分鐘撮合一次），日 K 的最高／最低價只是幾十次撮合的極值，掛在停損／目標的單未必真的撮得到；這些樣本不進主要比較，另列分盤撮合筆數。&#10;除權息當天若官方比率還沒發布（計算結果表約次一營業日才有），該單會暫停推進而不是拿事件前的停損價去比事件後的價格；等比率到齊會自動接著判，觀察天數不會被吃掉。">官方日 K 逐日補驗・雙觸保守記停損${data.dataGapCount ? `・${data.dataGapCount} 筆待補缺口` : ""}${data.corporateActionPendingCount ? `・${data.corporateActionPendingCount} 筆等官方除權息比率` : ""}${data.periodicCallCount ? `・${data.periodicCallCount} 筆分盤撮合` : ""}${data.haltedCount ? `・${data.haltedCount} 筆停牌中` : ""}${data.deferredExitCount ? `・${data.deferredExitCount} 筆跌停鎖死順延出場` : ""}${legacySamples ? `・舊版 ${legacySamples} 筆另存` : ""}${!measured && data.allVersions && data.allVersions.versions > 1 ? `・全版本合併 ${data.allVersions.winRate != null ? `${data.allVersions.winRate}%` : `${data.allVersions.wins}/${data.allVersions.resolved}`}（${data.allVersions.resolved} 筆結案）` : ""}</small>
    </div>
    ${renderVerificationMeasurement(data, 'swing')}
...
function renderTechnicalCorporateActions(data) {
  ...
  const eventList = info.events?.length
    ...
    // （目前連接的來源未涵蓋減資／面額變更），寫成「沒有公司行動」會是過度宣稱。
    : '<span title="偵測範圍：官方除權息參考價與公告公式，加上超過 10.5% 的跳空推估。目前連接的來源未涵蓋減資，幅度小於 10.5% 時可能偵測不到，圖上會保留原始跳空。">'
...
const GLOSSARY = [
  ...
  { term: "隔日沖（畫面）", aliases: ["隔日沖頁", "隔日沖"], cat: "畫面說明", def: "<strong>用途：</strong>看訊號日收盤後的三種短線型態，觀察實際下一交易日的慣性。<strong>當前數字：</strong>正式掃描最多 260 檔候選，每群最多 20 個訊號；每筆只對實際下一交易日，通常觀察約 1 個交易日。<strong>限制：</strong>「策略表現」是今天入選股票的歷史回看，有事後挑樣本偏誤；官方日 K 價格也不保證掛單成交。<strong>下一步：</strong>先切三群看市場位階與風險標籤，再開個股明細；判斷策略累積效果時看前向成績單，波段規劃改到「策略雷達」。" },
  ...
  { term: "盤中選股（畫面）", aliases: ["盤中選股"], cat: "畫面說明", def: "<strong>用途：</strong>盤中用動能與換手條件快速整理正在看的股票。<strong>當前數字：</strong>盤中約每 10 秒更新，只篩自選、隔日沖與搜尋等已載入股票池。<strong>限制：</strong>這是本機即時粗估，不是全市場掃描；資料來源失敗或不在盤中時不會假造即時結論。<strong>下一步：</strong>選動能／換手模式並調整清單篩選，開個股明細核對；要全市場波段型態與完整進出場計畫，改看「策略雷達」。" },
  { term: "策略雷達（畫面）", aliases: ["策略雷達"], cat: "畫面說明", def: "<strong>用途：</strong>從全市場流動性候選找中軌攻防與上軌續攻，提供進場、結構停損、目標與淨盈虧比。<strong>當前數字：</strong>每個交易日從上市櫃普通股取成交量前 240 檔，兩個場景各最多 40 個正式訊號；訊號日後滿 15 個官方交易日才屬成熟母體。淨獲利率與達標率各自使用該欄 20 筆有效樣本；信賴區間另需該欄 20 個不同有效日期，兩個門檻不能互借。<strong>限制：</strong>已結案但未成熟仍不進成熟比較；成熟但該欄缺值時保持未知，也不借別欄分母。日 K 觸價與假設含息結果都不是實際成交或帳戶績效。<strong>下一步：</strong>先看場景卡，再展開「分母、模型與來源」核對成熟、逐欄有效數與缺漏原因，最後才建立或調整自己的交易計畫。" },
  { term: "自選股（畫面）", aliases: ["自選股"], cat: "畫面說明", def: "<strong>用途：</strong>管理三組自選、查看行情摘要，並用交易帳本追蹤庫存與已實現損益。<strong>當前數字：</strong>登入後三組清單與帳本同步到伺服器；費稅來源分成估算、券商實際、手動與歷史資料。<strong>限制：</strong>費稅欄位留白才會依成交日、商品與預設券商方案估算；填入 0 元也是有效的實際值，紀錄上的來源標示用來區分估算、券商實際、手動與歷史。仍沒有自動當沖配對、先賣後買或多券商費率引擎。<strong>下一步：</strong>先登錄成交，再依券商對帳單填實際費稅；日後可用「修正」更新，清空實際值才會改回估算。" },
  { term: "技術分析（畫面）", aliases: ["技術分析"], cat: "畫面說明", def: "<strong>用途：</strong>把個股價格、量能、型態、基本面與公司概況放在同一頁核對。<strong>當前數字：</strong>可切日／週／月三種 K 線；「放大」支援縮放、逐根 OHLC 讀值與最多 40 筆趨勢線。<strong>限制：</strong>K 線先在日線層還原權息再聚合；公司行動資料不足時會抑制型態結論，指標與壓力支撐也不是買賣保證。<strong>下一步：</strong>輸入代號後先確認資料日期與來源，再切週期；需要逐根比較時開放大圖與資料閱讀區。" },
  { term: "處置看板（畫面）", aliases: ["處置看板"], cat: "畫面說明", def: "<strong>用途：</strong>集中查看官方風險公告與分盤交易狀態。<strong>當前數字：</strong>分成即將處置、處置中、即將出關、鉅額、注意、全額交割六類；處置期間可能每 5 或 20 分鐘撮合一次。<strong>限制：</strong>「新進／連 N 天／出關」依本機每日快照比對，伺服器沒開或來源不完整時只能顯示判定中；篩選後 0 檔不等於官方名單為空。<strong>下一步：</strong>先看分頁總數與資料警告，再用市場、分盤、搜尋或自選篩選，開卡片核對公告期間與最新可用報價。" },
  { term: "更多（畫面）", aliases: ["更多"], cat: "畫面說明", def: "<strong>用途：</strong>集中管理看盤設定、帳號與維護工具。<strong>當前數字：</strong>分成看盤設定與帳號與維護兩組，包含名詞、資料源、風險、提醒、帳號、個人備份、富邦 API、共享備註及版本。<strong>限制：</strong>個人資料需登入；版本比對或外部來源無法連線時會顯示無法確認，不會冒充最新。個人備份 v2 包含交易計畫，舊 v1 缺少計畫時會保留目前計畫。<strong>下一步：</strong>先登入並查看資料源狀態；要移機或回復資料時進「個人資料備份」，先下載，再用安全預覽確認取代範圍。" },
  // —— 看盤基礎 ——
  ...
  { term: "還原股價（除權息）", aliases: ["還原股價", "除權息", "除息", "除權"], cat: "風險與制度", def: "除權息當天股價會因配息／配股產生制度性跳空，<strong>不等於真的大跌</strong>。App 優先用官方現金股利、股票股利與現增資料還原歷史價；舊區段若只有大跳空可推估，會明示「<strong>疑似／估算還原</strong>」，不把推測冒充官方事件。官方公告欄位未齊時，策略雷達會暫停該檔判定，避免錯算均線。<br><strong>偵測範圍的界線</strong>：目前連接的官方來源以除權息為主，<strong>尚未涵蓋減資、面額變更、股票分割事件</strong>。這幾類事件靠「跳空超過 10.5%」推估——偵測到的大幅跳空會標成估算，但<strong>幅度小於 10.5% 的減資偵測不到</strong>，那段圖會保留原始跳空。所以技術分析頁寫「沒有偵測到公司行動」是指<strong>沒查到</strong>，不是保證沒發生。" },
