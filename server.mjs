...
async function loadDbOnce() {
  ...
  const migratedVerification = migrateVerificationMetadata({ signalSnapshots: dbCache.signalSnapshots || [], swingVerification: dbCache.swingVerification });
...
const TRADE_PLAN_SERVER_KEYS = ['createdAt', 'updatedAt', 'initial', 'activation', 'revisions', 'source', 'provenance', 'metadataRevisions'];
const TRADE_PLAN_METADATA_KEYS = ['tradeLinks', 'review'];
const TRADE_PLAN_REVIEW_DECISIONS = ['conditions-changed', 'execution-deviation', 'insufficient-data', 'no-entry'];
const TRADE_PLAN_LINK_LIMIT = 100;
function tradeLinkSnapshot(record) {
  const keys = ['id','code','market','brokerAccountId','side','shares','price','date','tradeDate','executedAt','session','instrumentType','grossAmountTwd','fee','tax','feeSource','taxSource','feeRuleId','taxRuleId'];
  return { ...Object.fromEntries(keys.map(key => [key, record[key] ?? null])),
    timePrecision: record.executedAt ? 'entered-timestamp' : 'date-only' };
}
function tradeLinkFingerprint(snapshot) { return createHash('sha256').update(stableJson(snapshot)).digest('hex'); }
function tradeLinkStatus(link, records) {
  const record = records.find(item => item.id === link.tradeId);
  return !record ? 'source-deleted' : tradeLinkFingerprint(tradeLinkSnapshot(record)) !== link.fingerprint ? 'source-changed' : 'valid';
}
function validateTradePlanReview(raw, prior, now, portable = false) {
  if (raw == null) return null;
  if (!raw || typeof raw !== 'object' || Array.isArray(raw) || Object.keys(raw).some(key => !['decision','reason','reviewedAt'].includes(key))
    || !TRADE_PLAN_REVIEW_DECISIONS.includes(raw.decision) || typeof raw.reason !== 'string' || raw.reason.length > 1000) throw tradePlanError('PLAN_REVIEW_INVALID', '請選擇檢討分類，說明上限 1000 字');
  const unchanged = prior && raw.decision === prior.decision && raw.reason === prior.reason;
  if (portable && (typeof raw.reviewedAt !== 'string' || !Number.isFinite(Date.parse(raw.reviewedAt)))) throw tradePlanError('BACKUP_PLAN_INVALID', '檢討時間不正確');
  if (!portable && Object.hasOwn(raw,'reviewedAt') && raw.reviewedAt !== prior?.reviewedAt) throw tradePlanError('PLAN_HISTORY_IMMUTABLE', '不可指定檢討時間');
  return { decision:raw.decision, reason:raw.reason, reviewedAt:portable ? raw.reviewedAt : unchanged ? prior.reviewedAt : now };
}
function validateSavedTradeLinks(links) {
  if (!Array.isArray(links) || links.length > TRADE_PLAN_LINK_LIMIT) throw tradePlanError('PLAN_LINK_INVALID', '每個計畫最多關聯 100 筆成交');
  const seen = new Set();
  for (const link of links) {
    if (!link || Object.keys(link).some(key=>!['tradeId','allocatedShares','snapshot','fingerprint','linkedAt'].includes(key))
      || typeof link.tradeId !== 'string' || !link.tradeId || link.tradeId.length > 48 || seen.has(link.tradeId)
      || !Number.isSafeInteger(link.allocatedShares) || link.allocatedShares <= 0
      || !link.snapshot || link.snapshot.id !== link.tradeId || !['buy','sell'].includes(link.snapshot.side)
      || !SECURITY_CODE_PATTERN.test(link.snapshot.code) || !['TWSE','TPEx'].includes(link.snapshot.market)
      || !isValidCompactCalendarDate(link.snapshot.date) || link.snapshot.tradeDate!==link.snapshot.date
      || typeof link.snapshot.brokerAccountId!=='string' || link.snapshot.brokerAccountId.length>48
      || !Number.isSafeInteger(link.snapshot.shares) || link.snapshot.shares < link.allocatedShares
      || !Number.isFinite(link.snapshot.price) || link.snapshot.price <= 0
      || ['grossAmountTwd','fee','tax'].some(key=>link.snapshot[key] !== null && (!Number.isFinite(link.snapshot[key]) || link.snapshot[key]<0))
      || stableJson(tradeLinkSnapshot(link.snapshot)) !== stableJson(link.snapshot)
      || tradeLinkFingerprint(link.snapshot) !== link.fingerprint || typeof link.linkedAt !== 'string' || !Number.isFinite(Date.parse(link.linkedAt))) throw tradePlanError('PLAN_LINK_INVALID', '成交關聯快照或份額不正確');
    seen.add(link.tradeId);
  }
...
function canonicalizeTradePlanMetadata(raw, prior, plan, records, now) {
  ...
  const links = supplied.map(link => {
    const old = prior?.tradeLinks?.find(item => item.tradeId === link?.tradeId);
    if (old && stableJson(link) === stableJson(old)) return cloneJson(old);
    if (!link || Object.keys(link).some(key=>!['tradeId','allocatedShares'].includes(key))
      || typeof link.tradeId !== 'string' || !Number.isSafeInteger(link.allocatedShares) || link.allocatedShares <= 0) throw tradePlanError('PLAN_LINK_INVALID', '新關聯只能指定成交與正整數股數');
    const record = records.find(item => item.id === link.tradeId);
    if (!record) throw tradePlanError('PLAN_LINK_SOURCE_MISSING', '本人帳本找不到來源成交');
    if (record.code !== plan.code || record.market !== plan.exchange || !['buy','sell'].includes(record.side)) throw tradePlanError('PLAN_LINK_IDENTITY_INVALID', '來源成交股票、市場或買賣別不符；未知市場須先在帳本確認');
    if (link.allocatedShares > record.shares) throw tradePlanError('PLAN_LINK_OVERALLOCATED', '關聯股數超過來源成交');
    const executed = record.executedAt ? Date.parse(record.executedAt) : null;
    if (executed !== null && (!Number.isFinite(executed) || !/(Z|[+-]\d{2}:\d{2})$/.test(record.executedAt)
      || executed > Date.parse(now) || toTaipeiCompactDate(new Date(executed)) !== record.date
      || (plan.provenance?.kind === 'server-authored' && executed < Date.parse(plan.createdAt)))) throw tradePlanError('PLAN_LINK_TIME_INVALID', '帳本輸入的成交時間早於本機計畫，或時間格式不完整');
    const snapshot = tradeLinkSnapshot(record);
  ...
  if (stableJson(links) !== stableJson(prior?.tradeLinks || []) || stableJson(review) !== stableJson(prior?.review || null)) {
    if (history.length >= TRADE_PLAN_REVISION_LIMIT) throw tradePlanError('PLAN_METADATA_LIMIT_EXCEEDED', '成交關聯與檢討已達 100 次修改上限，未修改資料；請匯出保留歷史');
    history.push({ revision:history.length+1, recordedAt:now, tradeLinks:cloneJson(links), review:cloneJson(review) });
  }
  if(prior && !Object.hasOwn(prior,'tradeLinks') && !Object.hasOwn(prior,'review') && !Object.hasOwn(prior,'metadataRevisions') && !links.length && !review && !history.length) return {};
  return {tradeLinks:links,review,metadataRevisions:history};
}
function assertTradePlanLinkAllocations(payload, records) {
  const totals = new Map();
  for (const plan of payload.plans) for (const link of plan.tradeLinks || []) {
    // 修正／刪除後的舊證據不佔新的有效份額，也不能阻止使用者檢討或解除。
    if (tradeLinkStatus(link,records) !== 'valid') continue;
    const total = (totals.get(link.tradeId) || 0) + link.allocatedShares;
    if (total > link.snapshot.shares) throw tradePlanError('PLAN_LINK_OVERALLOCATED', '此成交在所有本人計畫的有效分配總股數超過實際股數');
    totals.set(link.tradeId,total);
  }
}
function buildTradePlanLinkEvidence(payload, records = []) {
  return Object.fromEntries(payload.plans.map(plan => {
    const links = (plan.tradeLinks || []).map(link=>({...cloneJson(link),status:tradeLinkStatus(link,records)}));
    const valid = links.filter(link=>link.status==='valid');
    const buys = valid.filter(link=>link.snapshot.side==='buy'), sells=valid.filter(link=>link.snapshot.side==='sell');
    const sum = (items,key) => items.reduce((n,item)=>n+key(item),0);
    const buyShares=sum(buys,item=>item.allocatedShares),sellShares=sum(sells,item=>item.allocatedShares);
    const moneyKnown=valid.every(item=>Number.isFinite(item.snapshot.fee)&&Number.isFinite(item.snapshot.tax));
    const buyCash=moneyKnown?sum(buys,item=>((item.snapshot.grossAmountTwd ?? Math.round(item.snapshot.price*item.snapshot.shares*100)/100)+item.snapshot.fee+item.snapshot.tax)*item.allocatedShares/item.snapshot.shares):null;
    const sellCash=moneyKnown?sum(sells,item=>((item.snapshot.grossAmountTwd ?? Math.round(item.snapshot.price*item.snapshot.shares*100)/100)-item.snapshot.fee-item.snapshot.tax)*item.allocatedShares/item.snapshot.shares):null;
    const chronology=plan.provenance?.kind==='server-authored' && plan.activation && buys.length && buys.every(item=>item.snapshot.timePrecision==='entered-timestamp' && Date.parse(item.snapshot.executedAt)>Date.parse(plan.activation.recordedAt) && item.snapshot.price>plan.activation.intent.stopPrice);
    const completeLinks=links.length===valid.length;
    const reasons=['corporate-action-and-receivable-coverage-unknown'];
    if(!links.length)reasons.push('no-linked-trades');
    if(!completeLinks)reasons.push('invalidated-links');
    if(buyShares!==sellShares)reasons.push('open-or-partial-allocation');
    if(links.some(item=>item.snapshot.timePrecision==='date-only'))reasons.push('date-only-order-unknown');
    if(links.some(item=>!item.snapshot.brokerAccountId || ['default','unknown','legacy-unknown'].includes(item.snapshot.brokerAccountId)))reasons.push('broker-account-unconfirmed');
    if(!moneyKnown)reasons.push('fee-tax-unknown');
    return [plan.planId,{links,buyShares,sellShares,buyCash,sellCash,cashDifference:moneyKnown&&valid.length?sellCash-buyCash:null,
      originalRiskCash:completeLinks&&chronology?sum(buys,item=>(item.snapshot.price-plan.activation.intent.stopPrice)*item.allocatedShares):null,
      planReturn:null,netR:null,reasons,timeBasis:'entered-executedAt-not-independently-verified'}];
  }));
}
...
function canonicalizeTradePlans(input, existing = emptyTradePlans(), { db, records = [], now = new Date().toISOString() } = {}) {
  ...
  const plans = input.plans.map(raw => {
    ...
    if (Object.keys(raw).some(key => ![...TRADE_PLAN_IDENTITY_KEYS, ...TRADE_PLAN_INTENT_KEYS, ...TRADE_PLAN_SERVER_KEYS, ...TRADE_PLAN_METADATA_KEYS].includes(key))) throw tradePlanError('PLAN_INVALID', '計畫包含不支援或非本人可指定的欄位');
    ...
    return { ...economic, ...canonicalizeTradePlanMetadata(raw,prior,economic,records,now) };
  });
  ...
  assertTradePlanLinkAllocations(payload,records);
...
function validatePortableTradePlans(payload, now = new Date().toISOString()) {
  ...
  const plans = payload.plans.map(raw => {
    const identity = tradePlanIdentity(raw);
    if (Object.keys(raw).some(key => ![...TRADE_PLAN_IDENTITY_KEYS, ...TRADE_PLAN_INTENT_KEYS, ...TRADE_PLAN_SERVER_KEYS, ...TRADE_PLAN_METADATA_KEYS].includes(key))) throw tradePlanError('BACKUP_PLAN_INVALID', '備份計畫包含不支援的欄位');
    ...
    const metadataRevisions = raw.metadataRevisions ?? [];
    if (!Array.isArray(metadataRevisions) || metadataRevisions.length > TRADE_PLAN_REVISION_LIMIT) throw tradePlanError('BACKUP_PLAN_INVALID', '檢討歷史容量不正確');
    for (const [index, revision] of metadataRevisions.entries()) {
      if (!revision || Object.keys(revision).some(key=>!['revision','recordedAt','tradeLinks','review'].includes(key)) || revision.revision !== index+1 || typeof revision.recordedAt !== 'string' || !Number.isFinite(Date.parse(revision.recordedAt))
        || (index && Date.parse(revision.recordedAt)<Date.parse(metadataRevisions[index-1].recordedAt))) throw tradePlanError('BACKUP_PLAN_INVALID','檢討歷史時間或序號不一致');
    ...
    const metadata=['tradeLinks','review','metadataRevisions'].some(key=>Object.hasOwn(raw,key))?{tradeLinks,review,metadataRevisions:cloneJson(metadataRevisions)}:{};
    return { ...identity, ...cloneJson(previous.intent), ...metadata, createdAt: raw.createdAt, updatedAt: raw.updatedAt,
...
async function buildPersonalRestorePreview(db, user, session, bundle, options) {
  ...
  assertTradePlanLinkAllocations(tradePlans,trades.records);
  const restoredLinkEvidence=buildTradePlanLinkEvidence(tradePlans,trades.records);
  const invalidatedLinkCount=Object.values(restoredLinkEvidence).reduce((n,e)=>n+e.links.filter(link=>link.status!=='valid').length,0);
  ...
  const publicPlan = {
    sections: {
      tradePlans: { invalidatedLinkCount, mode: bundle.formatVersion === 1 ? 'preserve' : 'replace',
  ...
  companyDirectoryInFlight = (async () => {
    ...
    for (const marketKey of ["twse", "tpex"]) {
      ...
      markets[marketKey] = {
        ...
        observedAt: result.value?.fetchedAt || null,
        sourceAsOfPrecision: result.value?.asOf ? "date" : "unknown",
      };
...
async function fetchStockHistoryMonth(code, exchange, monthCompact, name = "", options = {}) {
  ...
  const task = (async () => {
    ...
    const now = Date.now();
    rows = rows.map(row => ({ ...row, observedAt: new Date(now).toISOString() }));
...
async function buildOvernightSignalsUncached({
  ...
  const enriched = await mapLimit(candidates, 3, async (quote, index) => {
    ...
    evidence.corporateActionsFingerprint = createHash("sha256").update(stableJson(officialActions)).digest("hex");
    evidence.issuedShares = issuedShares.get(quote.code) ?? null;
    ...
    if ((adjustments.unresolvedIndices || []).includes(history.length - 1)) {
      ...
      evidence.reason = "corporate-action-unresolved";
      return [];
    }
  ...
  if (!dateCompact && persistSnapshot) {
    try { body.publication = await saveSignalSnapshot(body); }
    catch { body.publication = { kind: 'not-persisted' }; body.warnings.push('正式隔日沖快照暫時無法儲存，將在下次更新重試。'); }
  }
  return body;
}
...
async function buildOvernightSignals({ dateCompact = "", maxPerGroup = 20, maxCandidates = 260, persistSnapshot = true } = {}) {
  ...
  const cacheKey = `${latestDate}:${maxPerGroup}:${maxCandidates}:${shouldPersistSnapshot ? "persist" : "read"}:${verificationModelKey(currentVerificationIdentity("overnight"))}`;
  const cached = getFreshTtlCacheEntry(overnightCache, cacheKey);
  if (cached) { cached.value = await confirmVerificationBody(cached.value); return cached.value; }
  ...
  const task = buildOvernightSignalsUncached({
    ...
    setBoundedDateCache(overnightCache, cacheKey, {
      expiresAt: Date.now() + (reference.coverageComplete && body.publication?.kind !== "not-persisted" ? 5 * 60 * 1000 : REFERENCE_RETRY_MS),
      value: body,
...
// T03：選股、觀察、成本與母體各自升版；舊證據不能反推當時用的模型。
function verificationIdentity(input = {}) {
  const value = input.identity || input;
  return Object.fromEntries([
    'snapshotSchemaVersion', 'selectionVersion', 'evaluationVersion', 'costModelVersion',
    'cohortPolicyVersion', 'entryModel', 'returnBasis',
  ].map(key => [key, value[key] ?? (key === 'selectionVersion' ? input.formulaVersion : undefined) ?? 'legacy-unknown']));
}

function verificationModelKey(identity) {
...
function priceVerificationIdentity(strategy) {
  return verificationIdentity({
    ...
    evaluationVersion: strategy === 'swing' ? 'swing-price-observation-v1' : 'overnight-price-observation-v1',
    costModelVersion: 'flat-round-trip-0.471pct-v1',
...
function migrateVerificationMetadata(input) {
  const db = cloneJson(input);
  for (const item of [...(db.signalSnapshots || []), ...Object.values(db.swingVerification || {}).flat()]) {
    item.identity = verificationIdentity(item);
    // 舊 observed 的原始證據不改寫；讀取時由 verificationIdentity 明示 unknown。
  }
  return db;
}

function canonicalVerificationScope(strategy) {
  return strategy === 'swing' ? { maxCandidates: 240, scenarioKey: '', limit: 40 } : { maxCandidates: 260, maxPerGroup: 20 };
}

function verificationPublicationKey(strategy, asOf, selectionVersion) {
  return stableJson({ strategy, tradeDate: compactToIsoDate(toCompactDate(asOf)), selectionVersion });
}

function sourceTimesFor(reference) {
  return Object.fromEntries(Object.entries(reference.markets || {}).map(([market, value]) => [market, {
    sourceAsOf: value.asOf || null, precision: value.asOf ? 'date' : 'unknown', observedAt: value.fetchedAt || null,
  }]));
}
...
function verificationInputContent(value) {
  ...
  return Object.fromEntries(Object.entries(value).filter(([key]) => !['observedAt','fetchedAt','generatedAt'].includes(key))
...
function publishVerification(db, strategy, body) {
  ...
  const canonical = stableJson(scope) === stableJson(canonicalVerificationScope(strategy));
  const identity = currentVerificationIdentity(strategy);
  identity.selectionVersion = body.formulaVersion || 'legacy-unknown';
  ...
  // 只對輸入與結果指紋化，計算時刻/取得時刻不會讓相同內容憑空長出新 revision。
  ...
  const captureId = createHash('sha256').update(stableJson({strategy,tradeDate,selectionVersion:identity.selectionVersion,scope,revision})).digest('hex');
  const capture = {
    captureId, publicationKey: key, strategy, tradeDate, identity, modelKey: verificationModelKey(identity),
    revision, kind, complete, requestScope: scope, inputFingerprint,
    generatedAt: body.generatedAt || null, publicationStartedAt,
    publishedAt: null, decisionAvailableAt: null, availableConfirmedAt: null,
    publicationTimePrecision: 'unknown', timingReason: 'availability-not-confirmed',
  ...
  if (kind === 'formal') {
    ...
    if (strategy === 'swing') {
      recordSwingVerification(db, { ...body, picks: capture.signals, publication: capture });
    } else {
      db.signalSnapshots ||= [];
      db.signalSnapshots.push({ asOf: tradeDate, savedAt: publicationStartedAt, publicationStartedAt, publishedAt: null, decisionAvailableAt: null,
        formulaVersion: identity.selectionVersion, identity, captureId, inputFingerprint, sourceTimes: capture.sourceTimes,
        coverage: capture.coverage, regime: body.regime || null, picks: capture.signals });
...
async function confirmVerificationPublication(publication) {
  ...
  try {
    return await commitDbMutation(db => {
      const capture = db.verificationPublications?.captures?.[publication?.captureId];
      ...
      const timing = {
        ...
        publicationTimePrecision: 'confirmed-available-upper-bound', timingReason: null,
      };
      const fillTiming = item => {
        if (item.availableConfirmedAt) return;
        for (const [key,value] of Object.entries(timing)) {
          if (item[key] == null || key === 'publicationTimePrecision' || key === 'timingReason') item[key] = value;
        }
      };
      fillTiming(capture);
      ...
      for (const snapshot of db.signalSnapshots || []) {
        ...
        fillTiming(snapshot);
        for (const signal of snapshot.picks || []) fillTiming(signal);
      }
      for (const entries of Object.values(db.swingVerification || {})) {
        for (const entry of entries) if (entry.captureId === capture.captureId) fillTiming(entry);
      }
      for (const snapshot of Object.values(db.swingSnapshots || {})) {
        if (snapshot.body?.publication?.captureId !== capture.captureId) continue;
...
async function confirmVerificationCaptures(captureIds) {
  const db = await loadDb();
  for (const captureId of new Set(captureIds.filter(Boolean))) {
    const capture = db.verificationPublications?.captures?.[captureId];
    if (capture && !capture.availableConfirmedAt) await confirmVerificationPublication(capture);
  }
}

async function confirmVerificationBody(body) {
  if (!body?.publication?.captureId || body.publication.availableConfirmedAt) return body;
  return { ...body, publication: await confirmVerificationPublication(body.publication) };
}
...
async function saveSignalSnapshot(body) {
  ...
  const publication = await commitDbMutation(db => publishVerification(db, 'overnight', { ...body, regime }));
  invalidateVerifyHistoryCacheForTest();
  return confirmVerificationPublication(publication);
}
...
function observationInputFingerprint(snapshot) {
  return snapshot.inputFingerprint || createHash('sha256').update(stableJson({ asOf: snapshot.asOf, picks: snapshot.picks || [] })).digest('hex');
}

function storedObservationFor(snapshot, expected = null) {
  const identity = expected || verificationIdentity(snapshot);
  const candidates = [snapshot?.observed, ...(snapshot?.observationRevisions || [])];
  for (const stored of candidates) {
    if (!stored || stored.complete !== true || stored.status !== 'final') continue;
    if (stored.formulaVersion !== overnightSnapshotFormulaVersion(snapshot)) continue;
    if (verificationModelKey(identity) !== verificationModelKey(verificationIdentity(stored))) continue;
    if ((stored.inputFingerprint || null) !== (expected ? observationInputFingerprint(snapshot) : snapshot.inputFingerprint || null)) continue;
    if (!Array.isArray(stored.rows)) continue;
    return { ...stored, identity: verificationIdentity(stored), observationPhase: 'final', warnings: Array.isArray(stored.warnings) ? stored.warnings : [] };
  }
  return null;
}
  ...
  await commitDbMutation((db) => {
    ...
    for (const { snapshot, observed } of entries) {
      ...
      const memo = {
        ...
        kind: snapshot.captureId ? 'forward-observation' : 'retrospective-observation',
        computedAt: observed.computedAt || new Date().toISOString(),
      ...
      if (snapshot.captureId && !item.observed) item.observed = memo;
      else (item.observationRevisions ||= []).push({ ...memo, revision: (item.observationRevisions?.length || 0) + 1 });
      changed = true;
    }
...
async function buildVerificationHistory() {
  ...
  const records = observations.map((observed, index) => {
    ...
    return {
      ...
      identity: observed.identity || verificationIdentity(snapshot), modelKey: verificationModelKey(observed.identity || verificationIdentity(snapshot)),
      captureId: snapshot.captureId || null, inputFingerprint: observationInputFingerprint(snapshot),
      publishedAt: snapshot.publishedAt || null, kind: observed.kind || (snapshot.captureId ? 'forward-observation' : 'legacy-observation'),
  ...
  const currentModelKey = verificationModelKey(currentVerificationIdentity('overnight'));
  const selectedModelKey = records.some(record => record.modelKey === currentModelKey) ? currentModelKey : records.at(-1)?.modelKey;
  const modelGroups = [...new Set(records.map(record => record.modelKey))].map(modelKey => ({ modelKey,
    identity: records.find(record => record.modelKey === modelKey).identity,
...
function recordSwingVerification(db, body) {
  ...
  const relevant = list.filter(entry => body.publication ? entry.captureId === body.publication.captureId : !entry.captureId);
  const seen = new Set(relevant.map((entry) => `${entry.exchange || ""}:${entry.code}:${entry.scenario}:${entry.formulaVersion || "legacy"}`));
  ...
  for (const pick of body.picks) {
    ...
    const key = `${pick.exchange || ""}:${pick.code}:${scenario}:${bodyVersion}`;
    ...
    list.push({
      ...
      ...(body.publication ? {
        identity: body.publication.identity, captureId: body.publication.captureId, signalId: pick.signalId,
...
async function buildSwingVerificationSummary() {
  ...
  const selectedModelKey = matchingEntries.some(entry => verificationModelKey(entry) === currentModelKey) ? currentModelKey
    : matchingEntries.length ? verificationModelKey(matchingEntries.at(-1)) : currentModelKey;
  ...
  for (const [day, entries] of Object.entries(store)) {
    for (const entry of entries) {
      ...
      if (formulaVersion !== SWING_FORMULA_VERSION || verificationModelKey(entry) !== selectedModelKey) continue;
  ...
  const scenarios = [...byScenario.values()].map((s) => {
    ...
    return {
      ...
      profitFactorNet: !knownCost ? null : (() => {
      ...
      nextOpenEntry: (() => {
        ...
        const wins = knownCost ? rows.filter((item) => netReturnPct(item.resultPctNextOpen) > 0).length : null;
        return {
          resolved: rows.length,
          wins,
          winRate: knownCost && rows.length >= WIN_RATE_MIN_SAMPLES ? Math.round((wins / rows.length) * 1000) / 10 : null,
  ...
  const body = {
    ...
    allVersions: (() => {
      ...
      return {
        ...
        resolved,
        wins,
        winRate: modelGroups.length === 1 && resolved >= WIN_RATE_MIN_SAMPLES ? Math.round((wins / resolved) * 1000) / 10 : null,
        reason: modelGroups.length > 1 ? "mixed-models" : null,
      };
    })(),
    scenarios,
  ...
  swingVerifySummaryCache = { key: cacheKey, expiresAt: Date.now() + 10 * 60 * 1000, value: body };
  return body;
}
...
async function scanSwingBoard(reference, latestDate, scenarioKey, maxCandidates) {
  ...
  const enriched = await mapLimit(candidates, 3, async (quote,index) => {
    try {
      ...
      history = appendTodayCloseBar(history, quote, latestDate);
      inputEvidence[index].historyFingerprint = createHash('sha256').update(stableJson(verificationInputContent(history))).digest('hex');
      inputEvidence[index].observedAt = history.map(row => row.observedAt).filter(Boolean).sort().at(-1) || null;
      ...
      inputEvidence[index].corporateActionsFingerprint = createHash('sha256').update(stableJson(officialActions)).digest('hex');
  ...
  const scanQuality = {
    candidateCount: candidates.length,
    completedCount: enrichedResults.length,
    historySuccessCount,
    freshHistoryCount,
    featureReadyCount,
    failedCount,
...
// forceRefresh 另記修訂；顯示可採較完整結果，首次正式發布及其驗證單永遠保留。
async function buildSwingBoard({ scenarioKey = "", limit = 40, maxCandidates = 240, forceRefresh = false } = {}) {
  ...
  const snapshotKey = maxCandidates === 240 ? `${latestDate}:all` : `${latestDate}:research:${maxCandidates}`;
  ...
  try {
    let persistedBody = await commitDbMutation((db) => {
      db.swingSnapshots ||= {};
      const publication = publishVerification(db, 'swing', body);
      const publishedBody = { ...body, publication };
      const existing = db.swingSnapshots[snapshotKey];
      ...
      if (!shouldReplace) return existing.body; // 修訂已在同一 draft 保存，顯示仍沿用較完整結果。
...
export {
  ...
  scanSwingBoard, buildSwingBoard, inspectSwingStock,
  // 波段前向驗證
