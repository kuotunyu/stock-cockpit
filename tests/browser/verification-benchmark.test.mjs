...
test('兩成績單候選池分母、未知與固定期間明細於四尺寸及200%可讀', {timeout:120000}, async()=>{
  ...
  try {
    ...
    for(const [strategy,url] of [['swing','/api/swing/verify'],['overnight','/api/overnight/verify/history']]) {
      ...
      const model={captureIdentity:{...payload.cohort?.models?.[0]?.identity,selectionVersion:strategy+'-original-selection',evaluationVersion:strategy+'-original-evaluation'},modelKey:strategy+'-full-fixed-model',horizon:strategy==='swing'?'next-open-to-session-15-close':'signal-close-to-next-close',benchmarkSpec:{version:'frozen-pool-fixed-price-v1'},
    ...
    for(const screen of ['strategy','overnight']) {
      ...
      assert.match(await fold.textContent(),/原採集模型/);assert.match(await fold.textContent(),screen==='strategy'?/swing-original-selection.*swing-original-evaluation/:/overnight-original-selection.*overnight-original-evaluation/);
      ...
      for(const width of [375,768,1280,1440])for(const factor of [1,2]) {
        ...
        if(width===375){await fold.locator(':scope > p').first().scrollIntoViewIfNeeded();await fixture.captureSnapshot(`benchmark-identity-${screen}-${width}-text${factor*100}`);}
      }
