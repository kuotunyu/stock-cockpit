...
| 第二層 D-20～D-33 | **3 完成 / 11 待決** | 真實缺陷，但修法牽涉口徑選擇 |
...
**下一步建議**：D-20、D-27 已完成（兩者都屬「只改數字的呈現與揭露、不動選股結果」）。
接著建議 **D-22**（帳本不處理除權／減資——除權日會在自己的持股上顯示假虧損，是使用者自己的錢，數字錯最有感；工程量最大但不改選股）。
**D-25／D-02／D-11 應該綁在一起做**：三者都會改變哪些標的入選並需升 `SWING_FORMULA_VERSION`／`OVERNIGHT_FORMULA_VERSION`，一次做完只讓統計基準斷一次。
...
> **2026-07-26 完成**：新增 `latestUpstreamDate(rows, getDate, notAfter)`，上櫃鉅額與變更交易改取「payload 內不晚於查詢日的最新日期」，非交易日不再整批消失；資料日落後查詢日時 push warning 明講實際日期。測試：`surveillance-tpex-lagging-date.test.mjs`（2 案），已突變抽查。
> **上市側維持不過濾**：BFIAUU／TWT85U 的 payload 根本沒有日期欄位（見 fixtures），沒有可比對的東西——這是上游的形狀限制，不是可修的 bug，不要為了「對稱」硬加。
> **刻意沒做**：完全沒有列時不發警告。那種情況分不出「當天真的沒有」與「上游還沒公布」，不編造結論。
...
> **2026-07-26 完成**：TWSE／TPEx 處置來源改夾帶 `code|起日|迄日`（沿用停牌來源既有的 `|` 慣例），日期窗在消費端套用。期間可解析且不涵蓋基準日 → 不標；解析不出來 → 保守保留（風險標籤寧可多標不要漏標）。測試：`risk-disposition-window.test.mjs`（3 案），已突變抽查。
> **為什麼不把過濾塞進 fetcher**：`riskSourceMemory` 以 source name 為鍵並持久化到 `risk-cache.json`，一旦結果跟日期相關就必須把 riskDate 加進 name（`TPEx 變更交易` 就是這樣），last-good 會變成每日獨立——上游失敗時反而沒有備援。夾帶期間、消費端過濾可同時保住快取與正確性。
> **向後相容**：舊 `risk-cache.json` 存的是純代號，讀到時 start/end 為空 → 保留。
> **未做**：把 `startsNextTradingDay` 帶到 pick 上（需要改 pick 的資料契約，留待有需求時）。
...
### ✅ D-20. 所有驗證／回測數字都是毛報酬，UI 零揭露

> **2026-07-26 完成，採選項 A（毛淨並陳）＋ C（文案揭露）**。使用者表明不熟金融、授權自行判斷，因此選了不動任何既有數字的方案：毛值維持原樣（既有測試與歷史快照不受影響），旁邊並陳伺服器算好的淨值。
> 新增 `VERIFY_ROUND_TRIP_COST_PCT = 0.471`（手續費 `TRADE_FEE_RATE × 0.6 折 × 2 邊` ＋ 賣出證交稅 3‰——隔日沖是「今日收盤買、次日賣」不是現股當沖，所以用全額 3‰）、`VERIFY_COST_NOTE` 與 `netReturnPct()`。
> 後端四處 summary 補淨值欄位：隔日沖單日／歷史 totals、回測分群與個股 recentBacktest、波段場景 `avgResultPctNet`；四個 notes 都加上揭露句。前端 `formatGrossWithNet()` 並陳，`.verify-net` 用 `--muted-2` ＋ `--fs-note` 低調呈現。
> 測試：`verify-net-return.test.mjs`（3 案）與 `verify-net-render.test.mjs`（4 案），已突變抽查。
> **實作時自己踩到 `Number(null)===0`**：`formatGrossWithNet` 第一版用 `Number.isFinite(Number(netPct))` 當守衛，缺值會渲染成「淨 +0.00%」——正是這個 repo 一路在修的造假零。已改用既有的 `finiteNumberOrNull()`，並由測試釘住。
> **已知限制（已寫進 note）**：驗證單只存 entry/stop/target、沒有股數與部位金額，所以這是**費率版**淨報酬，套不上 `computeTradeFee` 的每筆最低 20 元（成交金額低於約 23,400 元時實際成本更高），也未計滑價，不等同交易帳本口徑。
> **未做**：D-25 的 RR 門檻是否要改用淨值計算——那會改變哪些標的通過 `SWING_MIN_RR`，屬選股決策，仍待決定。
...
> **2026-07-26 完成（除權與現增；減資仍未支援）**。實作前先實測，發現問題比原稽核描述的更嚴重——**不只是顯示假虧損，而是功能性阻斷**：買 1000 股、配股 10% 後實際持有 1100 股，想賣出 1100 股會被 `buildPortfolio` 的賣超檢查擋下，錯誤訊息還叫使用者「請檢查買賣紀錄的日期與股數」，但紀錄是對的。
> 新增 `side: "corporateAction"` 紀錄型別（欄位 `stockRatio`／`subscriptionRatio`／`subscriptionPrice`，刻意與官方歸檔一致以便日後直接帶入）。會計上很單純：無償配股＝以 0 元取得股票、現增＝以認購價取得股票，都只是「加股數、加成本」，均價自然稀釋，所以 portfolio 的數學保持單純。不足一股無條件捨去（台股零股折現金發放，帳本只追蹤股數，寧可少算不要多算）。
> **驗證與正規化走完全獨立的分支**，不碰買賣／股利的既有規則——測試另外釘住「價格 0／股數 0 的買進仍必須被擋下」，確認沒有為了遷就新型別而放寬防線。正規化後的紀錄不帶假的 price/shares，避免下游誤當成交。
> 前端：除權基準日當天、持股有官方 `stockRatio` 且尚未登錄時，出現「記除權配股 · +N 股」快速鈕（沿用現金股利快速鈕的同一套模式，含 `.blur()`——「載入更多」踩過的焦點坑）；交易紀錄列表加獨立的精簡列，實測舊版渲染遇到這種紀錄會在 `shares.toLocaleString` 直接丟例外。
> 測試：`trades-corporate-action.test.mjs`（8 案），已突變抽查。
> **減資未支援**：本機官方歸檔不涵蓋減資（見 stock1-domain），使用者得手填；而且現金減資的成本處理（沖減成本 vs 認列已實現）是會計口徑選擇，需要使用者拍板。
> **事後補登（同日補上）**：`GET /api/trades` 新增 `missingCorporateActions`——伺服器用本機官方歸檔回溯比對，找出「官方有、帳本沒登錄」且使用者在基準日之前確實持有的除權／現增事件；庫存頁顯示提示與「補登」鈕，比率一律取自官方（手填最容易把「每仟股配股數」當成比率，驗證器擋的就是這個）。權利判定沿用除息既有的 `date < exDate` 規則：基準日當天買進不享有權利。純現金股利不影響股數，不混進清單。測試：`trades-missing-corporate-action.test.mjs`（7 案），已突變抽查。
> **仍存在的限制**：本機歸檔從部署後才開始累積，涵蓋不到部署前的歷史事件；減資完全不在歸檔內。
...
> **2026-07-26 完成，採選項 A（低於門檻不給百分比、不染色）＋ C（notes 說明分母組成）**。使用者授權自行判斷，門檻取 **20**。
> 後端 `WIN_RATE_MIN_SAMPLES = 20`：`resolved` 低於門檻時 `winRate` 回 null（語意是「還不足以當結論」不是「沒資料」），另回 `resolved` 與 `winRateMinSamples` 供前端顯示進度；notes 加上「分母只含已結案、達標／停損常 1~3 天結案而超時要等第 15 個交易日、同一天選出的標的高度共享大盤走勢」的說明。
> 前端場景卡：樣本不足顯示「累積中 N/20」且**不染紅綠**；一筆都還沒結案時仍退回 `--`（講「累積中 0/20」沒有意義）。
> 個股卡片旁的回測 chip 另用較寬鬆的門檻 **5**——它是個股層級的參考提示不是策略結論，次數本身照樣顯示讓使用者看得到樣本多小，但 1 次觸發不再給「+2 達成率 100%」。
> 測試：`tests/frontend/win-rate-min-samples.test.mjs`（7 案，含達門檻後照常染色的對照組），已突變抽查。
> **既有測試改寫**：`swing-verify.test.mjs` 原本釘 `winRate === 50`（結案 2 筆中 1 勝），稽核已預告需改寫；改為驗 `resolved === 2` 與 `winRate === null`，計數本身仍然釘住。
> **未做**：誤差範圍（cluster 版 Wilson 區間）。要正確處理「同日訊號高度相關」需要決定 cluster 定義與呈現方式，成本高且對使用者的實際幫助不如「不顯示不可靠的百分比」直接。
