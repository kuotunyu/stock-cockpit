...
庫存損益頁另有「我的成績單」：量的是你自己的成交（不是系統訊號）——本月／今年已實現、勝率、獲利因子、每筆平均、最長連虧、今年費稅與股利入帳、最近 12 個月逐月表，全部由伺服器從帳本算好回傳，未滿 20 筆只顯示累積中。最近兩週的成交會在最上面列出 T+2 交割款（買進應付價金＋手續費、賣出應收價金−費稅；開休市表未載入時只跳週末）。交易紀錄右上可匯出「帳本 CSV」與「已實現 CSV」（UTF-8 含 BOM，Excel 直接開）。
