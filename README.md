...
1. **選股訊號每日前向驗證 (Forward Validation)**：
   隔日沖與波段選股在收盤資料完整且兩市場日期對齊後保存正式快照，再以後續交易日的官方日 K 驗證；暫定結果不建立正式驗證紀錄。除了畫面請求，伺服器也每 10 分鐘檢查收盤任務（`SCHEDULER=off` 可關）；證交所整批收盤檔偶爾隔天早上才更新，這時會在下一個交易日補存前一個交易日的快照（只補前一天）；伺服器沒開的日子不會自動補建當日訊號。紀錄保存公式版本，成績單區分當前版本、樣本數、資料缺口與大盤位階。各引擎的統計口徑見下表。
...
庫存損益頁另有「我的成績單」：量的是你自己的成交（不是系統訊號）——本月／今年已實現、勝率、獲利因子、每筆平均、最長連虧、今年費稅與股利入帳、最近 12 個月逐月表，全部由伺服器從帳本算好回傳，未滿 20 筆只顯示累積中。最近兩週的成交會在最上面列出 T+2 交割款（買進應付價金＋手續費、賣出應收價金−費稅；開休市表未載入時只跳週末）。交易紀錄右上可匯出「帳本 CSV」與「已實現 CSV」（UTF-8 含 BOM，Excel 直接開）。成績單裡可設兩條停手線：本月虧損上限（元）與連虧停手（筆），存在這台裝置，達到就在庫存頁與隔日沖總覽顯示紀律提醒、記完賣出也會提醒；另有「今年稅務估算」（單筆 ≥ 2 萬股利的二代健保補充保費 2.11%、合併計稅 8.5% 可抵減上限 8 萬、28% 分離課稅、證交稅與手續費合計），皆為估算、非申報依據。
...
想讓它在登入 Windows 時自己開、掛了自己回來（伺服器沒開的交易日不會有收盤快照）：在 PowerShell 跑一次 `powershell -ExecutionPolicy Bypass -File scripts/register-autostart.ps1`。它登記工作排程「Stock1-server」：登入時以無視窗方式（`conhost --headless`）啟動 `scripts/stock1-watchdog.ps1` 常駐，每 10 分鐘看一次伺服器埠，沒在監聽就在背景重新啟動 `server.mjs`（當機會在 10 分鐘內拉回來），登記完也立刻啟動一次。伺服器與守門都沒有視窗，訊息都寫在 `DATA_DIR/logs/server-YYYYMMDD.log`（保留 14 天；`LOG_FILE=off` 可關）。暫停用 `-Stop`（下次登入自動恢復），移除自動啟動用 `-Unregister`。這是「登入才啟動」，電腦關機時仍不會採集。要更新程式：`git pull` 後在同一行指令後面加 `-Restart` 再跑一次，它會等伺服器手上的寫入落盤、停掉舊的，再用新程式啟動。
...
更新程式前先停止原伺服器並等待完整結束，更新完成後再啟動；Node 不會熱載。只改前端時，瀏覽器 **Ctrl+F5** 即可。
...
工作排程器也必須安排在服務已停止的時段；程式填 `npm.cmd` 完整路徑，引數填 `run backup "D:\OneDrive\stock1-backup"`，起始位置填專案資料夾。不要把失敗重跑當成熱備份。有登記守門（`register-autostart.ps1`）的話，服務停掉後 10 分鐘內會被重新啟動：手動備份前先跑一次加 `-Stop` 的登記指令（停掉守門與伺服器），備份完再跑一次不加參數的登記指令。
...
LOG_FILE=on       # 主控台訊息同步寫到 DATA_DIR/logs/server-YYYYMMDD.log（保留 14 天）；設 off 只留主控台
